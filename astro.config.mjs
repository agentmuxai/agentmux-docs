	...
	integrations: [
		starlight({
			...
			head: [
				...
				{
					tag: 'script',
					content: `document.addEventListener('DOMContentLoaded',function(){if(!matchMedia('(min-width: 50em)').matches)return;var s=document.getElementById('starlight__sidebar'),p=document.querySelector('sl-sidebar-state-persist');if(!s)return;try{var d=JSON.parse(sessionStorage.getItem('sl-sidebar-state')||'null');if(d&&p&&d.hash===p.dataset.hash&&typeof d.scroll==='number')s.scrollTop=d.scroll;}catch(e){}s.classList.add('sl-scroll-ready');});`,
				},
			],
