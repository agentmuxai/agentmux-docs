...
- **Accounts**: provider logins and API keys, managed in **≡ → Armory → Accounts**. You link one to an agent through the **Identity** field when you create it, or later with **Bind**. An agent with no account uses AgentMux's shared login folder for its provider. See [Auth flows](/auth/).
