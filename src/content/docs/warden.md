...
The Internet section is **closed by default**. Cross-network governance (via the AgentBus cloud relay) ships behind a future opt-in. The section currently shows a status chip of `disabled` and a one-line explanation.
